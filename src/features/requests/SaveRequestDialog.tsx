...
export function SaveRequestDialog({ suggestedName, onCancel, onConfirm }: SaveRequestDialogProps) {
  const { collections, createCollection } = useCollectionsStore();
  const [name, setName] = useState(suggestedName);
  const [collectionId, setCollectionId] = useState(collections[0]?.id ?? "");
  const [creatingCollection, setCreatingCollection] = useState(collections.length === 0);
  ...
  return (
    <div className="fixed inset-0 z-20 flex items-center justify-center bg-black/50">
      <div className="w-[420px] rounded-sm border border-surface-3 bg-surface-1 p-4">
        <h2 className="mb-3 text-sm font-semibold text-text-primary">Save Request</h2>
        ...
        <label className="mb-1 flex flex-col gap-1 text-sm">
          <span className="text-xs font-medium text-text-muted">COLLECTION</span>
          {!creatingCollection ? (
            <select
