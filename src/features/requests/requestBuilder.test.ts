...
describe("computeGeneratedHeaders", () => {
  it("shows the JSON content-type default", () => {
    const generated = computeGeneratedHeaders([], { type: "json", content: "{}" }, { type: "none" });
    expect(generated).toEqual([{ key: "Content-Type", value: "application/json" }]);
  });

  it("shows the Bearer authorization header", () => {
    const generated = computeGeneratedHeaders([], { type: "none" }, {
      type: "bearer",
      token: "{{token}}",
    });
    expect(generated).toEqual([{ key: "Authorization", value: "Bearer {{token}}" }]);
  });
